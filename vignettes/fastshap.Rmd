...
**NOTE:** Although [fastshap](https://cran.r-project.org/package=fastshap) efficiently employs the sampling approach to computing approximate Shapley values, the package also supports exact approaches for certain classes of linear models gradient boosted decision trees (e.g., [xgboost](https://cran.r-project.org/package=xgboost)).

## Example

The following example demonstrates the basic usage of the [fastshap](https://cran.r-project.org/package=fastshap) package.
...
With [fastshap](https://cran.r-project.org/package=fastshap) you can compute contributions for multiple features in parallel using any parallel backend supported by the [foreach](https://cran.r-project.org/package=foreach) package. This is illustrated in the code chunk below. 
...
You can compute the contributions of each feature for XGBoost models in an efficient way using the methods described in [(Lundberg 2017)](https://arxiv.org/abs/1705.07874). These are available through the `predict()` function for [xgboost](https://cran.r-project.org/package=xgboost) models; see `?xgboost::predict.xgb.Booster` for details. Below we compute the contributions for each feature using both methods and compare the results using a Shapley-based dependence plot on feature `x.3`, the results are quite surprising (__no parallel processing was used to obtain the [fastshap](https://cran.r-project.org/package=fastshap) results__). And remember, [fastshap](https://cran.r-project.org/package=fastshap) can be used with any prediction model in R. 
...
We can also check that [fastshap](https://cran.r-project.org/package=fastshap) converges to the true Shapley values by comparing the results to TreeSHAP while varying the number of Monte Carlo repetitions:
...
Finally, starting with [fastshap](https://cran.r-project.org/package=fastshap) version 0.0.4, you can request exact Shapley values for [xgboost](https://cran.r-project.org/package=xgboost) and linear models (i.e., models fit using `stats::lm()` and `stats::glm()`). This is illustrated in the code chunk below where we use `fastshap::explain()` to compute exact explanations using TreeSHAP from the previously fitted [xgboost](https://cran.r-project.org/package=xgboost) model.

```{r fastshap-exact, fig.width=7, fig.asp=0.9, out.width="70%"}
system.time({
