...
ames.trn <- subset(ames, Year_Sold <= 2008)
ames.tst <- subset(ames, Year_Sold > 2008)

# Features only
X.trn <- subset(ames.trn, select = -Sale_Price)
X.tst <- subset(ames.tst, select = -Sale_Price)

# Check column classes
table(unlist(sapply(ames, class)))
