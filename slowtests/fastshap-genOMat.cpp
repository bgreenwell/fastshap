...
LogicalMatrix genOMat2(int num_rows, int num_cols) {
  ...
  for (int i = 0; i < num_rows; i++) {
    ...
    // Construct row i of O
    O(i, _) = (sample(num_cols - 1, num_cols - 1, false) - 1) < num_feat[i];
...
LogicalMatrix genOMat3(int num_rows, int num_cols) {
  ...
  //
  // * Actually, this makes sense, right? Sampling with p = 0/1 is exact whereas
  //   anything in between is random
...
// [[Rcpp::export]]
LogicalMatrix genOMat4(int num_rows, int num_cols, int feature_col) {
...
// [[Rcpp::export]]
LogicalMatrix genOMat5(int num_rows, int num_cols) {
