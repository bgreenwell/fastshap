...
This repository contains comprehensive development guidelines for AI
agents in the `AGENTS.md` file. Please refer to that file for detailed
instructions on contributing to the `fastshap` R package.
...
- `devtools::check()` - Run full package validation (always run before
  committing)
- [`tinytest::test_all()`](https://rdrr.io/pkg/tinytest/man/run_test_dir.html) -
  Run all tests
- `tinytest::run_test_file("inst/tinytest/test-*.R")` - Run specific
  test files
...
- Always run `devtools::check()` to ensure package validity before
  committing

See `AGENTS.md` for complete development workflow and contribution
guidelines.
