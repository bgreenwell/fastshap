...
\CRANpkg{shapper} provides an R interface to the Python \pkg{shap} library \citep{lundberg-2017-KernelSHAP} using \CRANpkg{reticulate} \citep{R-reticulate}; however, it currently only supports KernelSHAP (\pkg{shap} itself suppors SampleSHAP, TreeSHAP, LinearSHAP, as well as various other model-specific Shapley explanation methods).
...
In python, there's \pkg{shap}. In julia, there's \pkg{SampleSHAP.jl} (a lightweight port of \pkg{fastshap}), \pkg{ShapML.jl}, and \pkg{ShapleyValues.jl}.
